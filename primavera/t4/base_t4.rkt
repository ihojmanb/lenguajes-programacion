...
(define (eval expr env sto)
  (match expr
    ...
    [(add l r)
     ...
     (def (v*s r-val r-sto) (eval r env sto))
    ...
    [(my-set x newval)
     (def (id idx) x)
     (def xloc (lookup-env idx env))  ;; location ox id x in env
     (def xval (lookup-sto xloc sto)) ;; current value in store
     (match sto
       [(mtSto) (error 'lookup-sto "free identifier ~a" x)]
       [(aSto loc val rest)  
        (begin
          (def (v*s eval-newval newvalSto) (eval newval env sto)) ;; should i propagate newvalSto?
          (if (equal? loc xloc) ;; search the right location
              (v*s (eval newval env sto) (aSto loc eval-newval rest))
              (eval (my-set x newval) env rest)))
        ]
       )]))
;; build-env-sto :: TODO
;; TODO TESTS
(define (build-env-sto idlist arglist env sto)
  (match arglist
    ...
    [_
     (def (v*s arg-val arg-sto) (eval (first arglist) env sto))
     (def new-loc (next-location arg-sto))
