...
(define (pretty-printing s [acc ""])
  (match s
    ; literals
    [(? number?) (printf"~a~n" (var-to-string s))]
    ;[(? boolean?) s]
    [(? string?) s]
    ;[(aEnv binding rest) s]
    [(? list?) (for-each pretty-printing s)]
    [(structV name var val)
     (begin
       ...
       (if (empty? val)
           ...
           (begin
             ;(string-append "{" (pretty-printing val) "}")
             (pretty-printing val)
             ))
       )
...
#;(test (pretty-printing (structV
                          'BinTree
                          'Node
                          (list 10 (structV 'BinTree 'Leaf '(7)) (structV 'BinTree 'Leaf '(2)))))
        "{Node {Leaf} {Leaf}}")
#|-----------------------------
